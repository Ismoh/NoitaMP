...
function throw_glass_shard( potion_entity, entity_file, pos_x, pos_y )
	...
	if entity ~= 0 then
		...
		component_write( EntityGetFirstComponent( entity, "PhysicsBodyComponent"), { on_death_really_leave_body=false} )
		component_write( EntityGetFirstComponent( entity, "LifetimeComponent"), { lifetime = 720 + Random(-120,120) } )
	end
end
