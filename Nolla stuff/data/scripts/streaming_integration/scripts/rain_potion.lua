...
local types = { "potion", "potion_aggressive", "potion_secret" }
...
for i=1,count do
	...
	if ( entity ~= nil ) and ( failed == false ) then
		...
		EntityLoad( "data/scripts/streaming_integration/entities/empty_circle_tiny_potion.xml", pos_x + off_x, pos_y + off_y )
	end
end
