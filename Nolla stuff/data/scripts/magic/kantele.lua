...
if ( #players > 0 ) then
	...
	if ( string.len(note) > 0 ) then
		...
		if ( do_portal_magic ) then
			local already_done = EntityHasTag( player_id, "kantele_secret_00" )
			...
			if ( already_done == false ) then
				...
				GameAddFlagRun( "kantele_secret_00" )
				EntityAddTag( player_id, "kantele_secret_00" )
			else
		...
		if ( do_worm_magic ) then
			local already_done = EntityHasTag( player_id, "kantele_secret_02" )
			
			if ( already_done == false ) then
				EntityLoad( "data/entities/animals/worm_big.xml", x, y - 64 )
				GamePrintImportant( "$log_kantele", "$logdesc_kantele" )
				GameAddFlagRun( "kantele_secret_02" )
				EntityAddTag( player_id, "kantele_secret_02" )
			else
				GamePrint( "$log_kantele_done" )
			end
		end
		...
		if ( do_alchemy_magic ) then
			...
			if ( #keys > 0 ) then
				for i,key_id in ipairs( keys ) do
					...
					local already_done = EntityHasTag( key_id, "alchemy_kantele" )
