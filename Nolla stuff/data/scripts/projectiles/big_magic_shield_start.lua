...
local rotation = Random(-10, 10)

while (math.abs(rotation) < 4) do
...
if ( shooter_id ~= nil ) and ( shooter_id ~= NULL_ENTITY ) then
	for i=1,how_many do
		...
		local part_id = shoot_projectile_from_projectile( entity_id, "data/entities/projectiles/deck/big_magic_shield_part.xml", new_x, new_y, 0, 0 )
