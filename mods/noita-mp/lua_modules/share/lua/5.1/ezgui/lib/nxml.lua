...
function TOKENIZER_FUNCS:skip_unsupported_elements()
    while not self:eof() do
        if self:match_string("<!") and not self:match_string("<!--") then
            self:move(2)
            while not self:eof() and self:match_string(">") do
                self:move()
            end
...
function TOKENIZER_FUNCS:read_whitespace()
    local start_idx = self.cur_idx
    local len = 0

    while not self:eof() and self:is_whitespace(self:cur_char()) do
        len = len + 1
        self:move()
    end
    local ws = str_sub(self.data, start_idx, len)
    return str_sub(self.data, start_idx, len)
end

function TOKENIZER_FUNCS:read_comment()
    local start_idx = self.cur_idx - 1
    local len = 4 -- The characters "<!--"

    self:move(3) -- "<" has already been consumed
    while not self:eof() and not self:match_string("-->") do
        len = len + 1
        self:move()
    end
...
local C_CARRIAGERETURN = string.byte("\r")
local C_NEWLINE = string.byte("\n")

function TOKENIZER_FUNCS:next_token()
    self:skip_unsupported_elements()
    ...
    elseif c == C_LT then
        if self:match_string("!--") then
            local idx = self.cur_idx - 1
            return { type = "comment", value = self:read_comment(), pos = idx }
        else
            return { type = "<", pos = self.cur_idx - 1 }
        end
    elseif c == C_GT then return { type = ">", pos = self.cur_idx - 1 }
    elseif c == C_SLASH then return { type = "/", pos = self.cur_idx - 1  }
    elseif c == C_EQ then return { type = "=", pos = self.cur_idx - 1  }
    elseif c == C_QUOTE then
        local idx = self.cur_idx - 1
        return { type = "quoted_string", value = self:read_quoted_string(), pos = idx }
    elseif c == C_SPACE or c == C_TAB or c == C_NEWLINE or c == C_CARRIAGERETURN then
        self.cur_idx = self.cur_idx - 1
        local idx = self.cur_idx
        return { type = "whitespace", value = self:read_whitespace(), pos = idx }
    else
        local idx = self.cur_idx
        return { type = "string", value = self:read_unquoted_string(), pos = idx }
    end
end
...
function PARSER_FUNCS:parse_attr(attr_table, name)
    ...
    if tok.type == "=" then
        ...
        if tok.type == "quoted_string" or tok.type == "string" then
...
function PARSER_FUNCS:skip_whitespace_in_front_of_token(token)
...
function PARSER_FUNCS:get_next_non_ws_non_comment_token()
    local token = self.tok:next_token()
    while token and (token.type == "whitespace" or token.type == "comment") do
        token = self.tok:next_token()
    end
    return token
end

function PARSER_FUNCS:get_next_non_comment_token()
    local token = self.tok:next_token()
    while token and token.type == "comment" do
        token = self.tok:next_token()
    end
    return token
end
...
function PARSER_FUNCS:parse_element(skip_opening_tag)
    ...
    if not skip_opening_tag then
        tok = self:get_next_non_ws_non_comment_token()
        if tok.type ~= "<" then
    ...
    while true do
        tok = self:get_next_non_comment_token()
        local whitespace_tokens = {}
        while tok and tok.type == "whitespace" do
            table.insert(whitespace_tokens, tok)
            tok = self.tok:next_token()
        end
        ...
        elseif tok.type == "<" then
            if self.tok:cur_char() == C_SLASH then
                ...
                local end_name = self:skip_whitespace_in_front_of_token(self.tok:next_token())
                if end_name.type == "string" and end_name.value == elem_name then
                    local close_greater = self:skip_whitespace_in_front_of_token(self.tok:next_token())
            ...
            for i, tok in ipairs(whitespace_tokens) do
                content_idx = content_idx + 1
                elem.content[content_idx] = tok
            end
            content_idx = content_idx + 1
            elem.content[content_idx] = tok
        end
...
function PARSER_FUNCS:parse_elements()
    local tok = self:get_next_non_ws_non_comment_token()
    local elems = {}
    local elems_i = 1
...
-- Returns:
-- 1. The text content of an element, including whitespace
-- 2. The cursor position of the first character of the string, starting at the first whitespace
function XML_ELEMENT_FUNCS:text()
    ...
    local text = self.content[1].value
    for i = 2, content_count do
        local elem = self.content[i]
        if elem.type ~= "comment" then
            if elem.type == "quoted_string" then
                text = text .. "\"" .. (elem.value or elem.type) .. "\""
            else
                text = text .. (elem.value or elem.type)
            end
        end
    end
    return text, self.content[1].pos
end
...
function nxml.tostring(elem, packed, indent_char, cur_indent)
    ...
    if elem.content and #elem.content ~= 0 then
        ...
        s = s .. strip_whitespace(elem:text())
    end
