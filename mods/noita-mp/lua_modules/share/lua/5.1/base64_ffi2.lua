...
function base64.encode(str)
    ...
    crypt.CryptBinaryToStringA(str, #str, 1, nil, buflen)

    local buf = ffi.new("char[?]", buflen[0])
    crypt.CryptBinaryToStringA(str, #str, 1, buf, buflen)
    return ffi.string(buf, buflen[0])
end

function base64.decode(str)
    local buflen = ffi.new("int[1]")
    crypt.CryptStringToBinaryA(str, #str, 1, nil, buflen, nil, nil)

    local buf = ffi.new("char[?]", buflen[0])
    crypt.CryptStringToBinaryA(str, #str, 1, buf, buflen, nil, nil)
    return ffi.string(buf, buflen[0])
end
