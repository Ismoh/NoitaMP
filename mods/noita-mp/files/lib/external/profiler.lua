...
local config               = {
    ...
    fW          = 90, --20, -- Width of the file column
    fnW         = 35, --28, -- Width of the function name column
...
local function singleSearchReturn(inputString, search)
    for _ in string.gmatch(inputString, search) do
        -- luacheck: ignore
        return true
    end
    return false
...
function module.report(filename)
    if not stopTime or isRunning then
        module.stop()
    end
